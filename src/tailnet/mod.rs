...
fn ssh_config_contents(
    ...
    // No `Include ~/.ssh/config`: ssh treats an unreadable include as a
    // fatal configuration error, and sandboxes commonly deny ~/.ssh. Callers
    // who want their own config can add `-F` twice or a ProxyCommand option.
    format!(
        "# Generated by temps-agent-runtime for tailnet \"{}\". Do not edit; it is rewritten on every status refresh.\n\
         # Self-contained on purpose: combine with your own settings via extra -o options.\n\
         Host {}\n\
